	...
	<view class="page-body" :style="'height:'+height+'px'">
		...
		<scroll-view class="nav-right" scroll-y :scroll-top="scrollTop" @scroll="scroll" :style="'height:'+height+'px'" scroll-with-animation>
			<view v-for="(foods,index) in classifyData" :key="index" class="box">
				<view class="" >
					<image src="../../static/images/demo/demo4.jpg" mode="widthFix" style="height: 200upx;" @click="cart()"></image>
				</view>
				<view class="py-2" style="font-size: 20px;">
					{{ foods.name }}
				</view>
				
					<view :id="i==0?'first':''" class="nav-right-item" v-for="(item,i) in foods.foods" :key="i">
						<navigator url="../detail/detail?detail=">
							<view class="">
								<image :src="item.icon" />
								<view>{{item.name}}</view>
							</view>
	...
	// 					rect.height  // 节点的高度
			...
			scroll(e) {
				...
				this.timeoutId = setTimeout(function(){ //节流
					_this.scrollHeight = e.detail.scrollTop + 1 + _this.height/3;
